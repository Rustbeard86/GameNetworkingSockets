...
void CCrypto::Init() {
  ...
  static bool once = []() {
    ...
    BCryptOpenAlgorithmProvider(&hAlgRandom, BCRYPT_RNG_ALGORITHM, nullptr, 0);
    AssertFatal(hAlgRandom != INVALID_HANDLE_VALUE);

    BCryptOpenAlgorithmProvider(&hAlgSHA256, BCRYPT_SHA256_ALGORITHM, nullptr, 0);
    ...
    BCryptOpenAlgorithmProvider(&hAlgHMACSHA256, BCRYPT_SHA256_ALGORITHM, nullptr, BCRYPT_ALG_HANDLE_HMAC_FLAG);
    AssertFatal(hAlgHMACSHA256 != INVALID_HANDLE_VALUE);
    BCryptGetProperty(hAlgHMACSHA256, BCRYPT_OBJECT_LENGTH, (PUCHAR)&cbBufferHMACSHA256, sizeof(cbBufferHMACSHA256),
                      &garbage, 0);
    AssertFatal(cbBufferHMACSHA256 > 0 && cbBufferHMACSHA256 < 16 * 1024 * 1024);

    BCryptOpenAlgorithmProvider(&hAlgHMACSHA1, BCRYPT_SHA1_ALGORITHM, nullptr, BCRYPT_ALG_HANDLE_HMAC_FLAG);
    AssertFatal(hAlgHMACSHA1 != INVALID_HANDLE_VALUE);
    BCryptGetProperty(hAlgHMACSHA1, BCRYPT_OBJECT_LENGTH, (PUCHAR)&cbBufferHMACSHA1, sizeof(cbBufferHMACSHA1), &garbage,
                      0);
...
bool AES_GCM_CipherContext::InitCipher(const void* pKey, size_t cbKey, size_t cbIV, size_t cbTag, bool bEncrypt) {
  ...
  if (BCryptSetProperty(ctx->hAlgAES, BCRYPT_CHAINING_MODE, (PUCHAR)BCRYPT_CHAIN_MODE_GCM,
  ...
  if ((ret = BCryptGenerateSymmetricKey(ctx->hAlgAES, &ctx->hKey, ctx->pbKeyObject, ctx->cbKeyObject, (PUCHAR)pKey,
                                        (ULONG)cbKey, 0)) != 0)
    return false;
...
bool AES_GCM_EncryptContext::Encrypt(
    const void* pPlaintextData, size_t cbPlaintextData, const void* pIV, void* pEncryptedDataAndTag,
    uint32* pcbEncryptedDataAndTag, const void* pAdditionalAuthenticationData,
    size_t cbAuthenticationData // Optional additional authentication data.  Not encrypted, but will be included in the
                                // tag, so it can be authenticated.
) {
  BCryptContext* ctx = (BCryptContext*)(this->m_ctx);
  ...
  ULONG ct_size;
  NTSTATUS status = BCryptEncrypt(ctx->hKey, (PUCHAR)pPlaintextData, (ULONG)cbPlaintextData, &paddingInfo, NULL, 0,
                                  (PUCHAR)pEncryptedDataAndTag, *pcbEncryptedDataAndTag, &ct_size, 0);
...
bool AES_GCM_DecryptContext::Decrypt(const void* pEncryptedDataAndTag, size_t cbEncryptedDataAndTag, const void* pIV,
  ...
  paddingInfo.pbAuthData = cbAuthenticationData ? (PUCHAR)pAdditionalAuthenticationData : NULL;
  ULONG pt_size;
  NTSTATUS status = BCryptDecrypt(ctx->hKey, (PUCHAR)pEncryptedDataAndTag, (ULONG)cbEncryptedDataAndTag, &paddingInfo,
                                  NULL, 0, (PUCHAR)pPlaintextData, *pcbPlaintextData, &pt_size, 0);
...
void CCrypto::GenerateRandomBlock(void* pvDest, int cubDest) {
  ...
  NTSTATUS status = BCryptGenRandom(hAlgRandom, (PUCHAR)pvDest, (ULONG)cubDest, 0);
  AssertFatal(NT_SUCCESS(status));
}
...
//-----------------------------------------------------------------------------
void CCrypto::GenerateHMAC256(const uint8* pubData, uint32 cubData, const uint8* pubKey, uint32 cubKey,
                              SHA256Digest_t* pOutputDigest) {
...
//-----------------------------------------------------------------------------
void CCrypto::GenerateHMAC(const uint8* pubData, uint32 cubData, const uint8* pubKey, uint32 cubKey,
  ...
  status = BCryptHashData(hHash, (PUCHAR)pubData, (ULONG)cubData, 0);
  AssertFatal(NT_SUCCESS(status));
  status = BCryptFinishHash(hHash, *pOutputDigest, sizeof(SHADigest_t),
                            0); // Fix: was sizeof(SHA256Digest_t) — 32 bytes overflows 20-byte SHA1 digest
  AssertFatal(NT_SUCCESS(status));
  status = BCryptDestroyHash(hHash);
  AssertFatal(NT_SUCCESS(status));
}
